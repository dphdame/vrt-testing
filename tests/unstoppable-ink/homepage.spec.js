...
const SITE_URL = 'https://unstoppable.ink';
const SITE_NAME = 'unstoppable-ink';
...
test.describe(`${SITE_NAME} Homepage Visual Regression`, () => {
  ...
  test('navbar layout', async ({ page }) => {
    const navbar = page.locator('nav, .navbar, .navbar-container, header').first();
  ...
  test('hero section', async ({ page }) => {
    const hero = page.locator('.landing-hero, .hero, [class*="hero"], .site-header').first();
...
test.describe(`${SITE_NAME} CSS Validation`, () => {
  ...
  test('CSS rules are being parsed correctly', async ({ page }) => {
    ...
    // Should have substantial CSS rules - if too few, check for syntax errors
